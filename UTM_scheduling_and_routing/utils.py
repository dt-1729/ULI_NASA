...
def generate_points_exact(
    ...
    """
    Generate exactly n_nodes points with minimum pairwise
    ...
    for _ in range(max_attempts):
        ...
        # Find existing points within min_spacing
    ...
    if len(points) < n_nodes:
        raise RuntimeError(
            "Cannot fit {} points with "
            "min_spacing={} into {}x{} region. "
            "Reduce min_spacing or enlarge the domain."
            .format(
                n_nodes,
                min_spacing,
                width,
...
def generate_random_graph(params):
    """
    Parameters
    ----------
    ...
    long_edge_probability = params['long_edge_probability']
    long_edge_decay = params['long_edge_decay']

    points = generate_points_exact(
        n_nodes=n_nodes,
        width=width,
        height=height,
...
def get_cbf_mode(name, tolArray):
    if name == "el":
        # ellispoid cbf mode
        mode = {
            'mode':'ellipse',
...
def set_mep_opt_config(solver_name):
    ...
    return optim_config, anneal_config
